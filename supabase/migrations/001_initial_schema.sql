...
CREATE TABLE initra.project_templates (
...
CREATE INDEX idx_templates_category ON initra.project_templates(category);
CREATE INDEX idx_templates_slug ON initra.project_templates(slug);

-- ── Template Stack Options ──────────────────────
-- Dynamic form fields per template
CREATE TABLE initra.template_stack_options (
  id UUID PRIMARY KEY DEFAULT extensions.uuid_generate_v4(),
...
CREATE INDEX idx_stack_options_template ON initra.template_stack_options(template_id);

-- ── IDE Targets ─────────────────────────────────
-- Supported IDE configurations
CREATE TABLE initra.ide_targets (
  id UUID PRIMARY KEY DEFAULT extensions.uuid_generate_v4(),
  slug TEXT UNIQUE NOT NULL,
  name TEXT NOT NULL,
  logo_url TEXT,
...
CREATE TABLE initra.wizard_sessions (
  id UUID PRIMARY KEY DEFAULT extensions.uuid_generate_v4(),
...
CREATE INDEX idx_sessions_user ON initra.wizard_sessions(user_id);
CREATE INDEX idx_sessions_share ON initra.wizard_sessions(share_slug);

-- ── Generated Files ─────────────────────────────
-- Output files from prompt engine
CREATE TABLE initra.generated_files (
  id UUID PRIMARY KEY DEFAULT extensions.uuid_generate_v4(),
...
CREATE INDEX idx_generated_session ON initra.generated_files(session_id);

-- ── Community Projects ──────────────────────────
-- User-suggested open source projects
CREATE TABLE initra.community_projects (
  id UUID PRIMARY KEY DEFAULT extensions.uuid_generate_v4(),
...
CREATE INDEX idx_community_status ON initra.community_projects(status);
CREATE INDEX idx_community_votes ON initra.community_projects(vote_score DESC);
CREATE INDEX idx_community_category ON initra.community_projects(category);

-- ── Project Votes ───────────────────────────────
-- Upvote/downvote on community projects
CREATE TABLE initra.project_votes (
  id UUID PRIMARY KEY DEFAULT extensions.uuid_generate_v4(),
...
CREATE INDEX idx_votes_project ON initra.project_votes(project_id);
CREATE INDEX idx_votes_user ON initra.project_votes(user_id);
...
CREATE TABLE initra.agent_contributions (
  id UUID PRIMARY KEY DEFAULT extensions.uuid_generate_v4(),
  user_id UUID NOT NULL REFERENCES initra.profiles(id) ON DELETE CASCADE,
  project_id UUID NOT NULL REFERENCES initra.community_projects(id) ON DELETE CASCADE,
  ide_target TEXT NOT NULL,
...
CREATE INDEX idx_contributions_project ON initra.agent_contributions(project_id);
CREATE INDEX idx_contributions_user ON initra.agent_contributions(user_id);
...
CREATE TABLE initra.prompt_templates (
  id UUID PRIMARY KEY DEFAULT extensions.uuid_generate_v4(),
...
CREATE INDEX idx_prompt_templates_lookup ON initra.prompt_templates(ide_slug, project_category, is_active);

-- ── Row Level Security ──────────────────────────

-- Profiles: users can read all, update own
ALTER TABLE initra.profiles ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Public profiles are viewable by everyone" ON initra.profiles
  FOR SELECT USING (true);
CREATE POLICY "Users can update own profile" ON initra.profiles
  FOR UPDATE USING (auth.uid() = id);

-- Project Templates: public read
ALTER TABLE initra.project_templates ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Templates are viewable by everyone" ON initra.project_templates
  FOR SELECT USING (true);

-- Template Stack Options: public read
ALTER TABLE initra.template_stack_options ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Stack options are viewable by everyone" ON initra.template_stack_options
  FOR SELECT USING (true);

-- IDE Targets: public read
ALTER TABLE initra.ide_targets ENABLE ROW LEVEL SECURITY;
CREATE POLICY "IDE targets are viewable by everyone" ON initra.ide_targets
  FOR SELECT USING (true);

-- Wizard Sessions: users can CRUD own, read public
ALTER TABLE initra.wizard_sessions ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can view own sessions" ON initra.wizard_sessions
  FOR SELECT USING (auth.uid() = user_id OR is_public = true);
CREATE POLICY "Users can create sessions" ON initra.wizard_sessions
  FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Anonymous sessions allowed" ON initra.wizard_sessions
  FOR INSERT WITH CHECK (user_id IS NULL);

-- Generated Files: viewable if session is accessible
ALTER TABLE initra.generated_files ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Files viewable with session access" ON initra.generated_files
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "Files insertable by session owner" ON initra.generated_files
...
CREATE POLICY "Community projects are viewable by everyone" ON initra.community_projects
  FOR SELECT USING (true);
CREATE POLICY "Authenticated users can suggest projects" ON initra.community_projects
  FOR INSERT WITH CHECK (auth.uid() = suggested_by);
CREATE POLICY "Users can update own projects" ON initra.community_projects
...
CREATE POLICY "Users can view own votes" ON initra.project_votes
  FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can cast votes" ON initra.project_votes
  FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can change own votes" ON initra.project_votes
  FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can remove own votes" ON initra.project_votes
...
CREATE POLICY "Contributions are viewable by everyone" ON initra.agent_contributions
  FOR SELECT USING (true);
CREATE POLICY "Users can submit contributions" ON initra.agent_contributions
  FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update own contributions" ON initra.agent_contributions
  FOR UPDATE USING (auth.uid() = user_id);

-- Prompt Templates: public read
ALTER TABLE initra.prompt_templates ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Prompt templates are viewable" ON initra.prompt_templates
  FOR SELECT USING (true);
